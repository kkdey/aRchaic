...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--fasta", required=True, help = "reference file, the reference file must be indexed and the same one used to generate the bam files")
    ...
    for chr in chrs:
        ...
        for read in samfile.fetch(('chr' + str(chr)) if args.add_chr else str(chr)):
            ...
            for i in range(len(posg)):
                ...
                patt  = (ref[0:(nflanking+1)]).upper() + '->' + mut.upper() + (ref[(nflanking+1):(2*nflanking+1)]).upper()
