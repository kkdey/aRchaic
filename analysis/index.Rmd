...
The model output, in particular **model$omega** and **model$theta** can be visualized using 
...
memberships in **model$omega** in a stacked bar chart (see Fig 1 left).

Also, the `output_dir` should contain for each $k=1,2$, a `logo_clus_k.png` file
visualizing the mismatch profile for each cluster as in **model$theta**. If
